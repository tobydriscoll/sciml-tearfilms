...
function load_data(batchsize, train_split=0.8, n_comp=20)
    ...
    pcaI = fit(PCA, data_I, maxoutdim=n_comp, pratio=0.9999);
    pcaT = fit(PCA, data_T, maxoutdim=n_comp, pratio=0.9999);
    n_comp = min(n_comp, length(pcaI.prinvars) + length(pcaT.prinvars))
    pcac = fit(PCA, data_c, maxoutdim=n_comp, pratio=0.9999);

    X = [predict(pcaI, data_I); predict(pcaT, data_T)];
    y = predict(pcac, data_c);
    (X_train, y_train), (X_test, y_test) = splitobs((X, y); at=train_split)
    t = range(0, 1, size(data_I, 1))
    return (
        t, pcaI, pcaT, pcac,
...
t, pcaI, pcaT, pcac, train_loader, test_loader = load_data(batch_size)
d_input = size(train_loader.data[1], 1)
d_output = size(train_loader.data[2], 1)
model = Chain(
    Dense(d_input => 256, relu),
    Dense(256 => 256, relu),
    Dense(256 => 256, relu),
    Dense(256 => 128, relu),
    Dense(128 => d_output),
...
σI = sqrt.(pcaI.prinvars)
σT = sqrt.(pcaT.prinvars)
σc = sqrt.(pcac.prinvars)
##
...
# loss = MSELoss(; agg = x -> mean(σc .* x))
loss = MSELoss()

##

train_acc, test_acc = train(loss, model, ps, st, device(train_loader), device(test_loader); num_epochs=50, rate=0.0004)
