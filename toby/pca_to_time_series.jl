...
function load_data(files)
    ...
    return t, (;I = data_I, T = data_T, c = data_c)
end
...
pca, train_loader, test_loader = convert_data(data, batch_size)
d_input = size(train_loader.data[1], 1)
d_output = size(train_loader.data[2], 1)
model = Chain(
    Dense(d_input => 256, tanh),
    Dense(256 => 256, tanh),
    Dense(256 => 256, tanh),
    Dense(256 => d_output),
)
...
train_acc, test_acc = train(loss, model, ps, st, device(train_loader), device(test_loader); num_epochs=200, rate=0.0004)

##

test_err = []
for (X, c_true) in test_loader
    c_pred = first(model(X, ps, st))
...
predicted = first(model(X, ps, st))
a = predicted[:,099]; b =y[:,099]
# a[10:end].=0
# a[6:end] .= b[6:end]
fig, ax, _ = lines(t, a, label="predicted")
lines!(t, b, label="actual")
